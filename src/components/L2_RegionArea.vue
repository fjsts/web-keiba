<template>
  <div class="py-3">
    <span class="py-5 text-sm">3.競馬場を選択してください</span>

    <TabGroup>
      <TabList>
        <Tab id="kawasaki" class="w-1/3 selction-btn text-xs">
          川崎<br>(<span id="kawasaki_count"></span>件)
        </Tab>
        <Tab id="nagoya" class="w-1/3 selction-btn text-xs">
          名古屋<br>(<span id="nagoya_count"></span>件)
        </Tab>
        <Tab id="kochi" class="w-1/3 selction-btn text-xs">
          高知<br>(<span id="kochi_count"></span>件)
        </Tab>
        <Tab id="mizusawa" class="w-1/3 selction-btn text-xs">
          水沢<br>(<span id="mizusawa_count"></span>件)
        </Tab>
        <Tab id="sonoda" class="w-1/3 selction-btn text-xs">
          園田<br>(<span id="sonoda_count"></span>件)
        </Tab>
        <Tab id="saga" class="w-1/3 selction-btn text-xs">
          佐賀<br>(<span id="saga_count"></span>件)
        </Tab>
        <Tab id="funabashi" class="w-1/3 selction-btn text-xs">
          船橋<br>(<span id="funabashi_count"></span>件)
        </Tab>
        <Tab id="kasamatsu" class="w-1/3 selction-btn text-xs">
          笠松<br>(<span id="kasamatsu_count"></span>件)
        </Tab>
        <Tab id="urawa" class="w-1/3 selction-btn text-xs">
          浦和<br>(<span id="urawa_count"></span>件)
        </Tab>
        <Tab id="himeji" class="w-1/3 selction-btn text-xs">
          姫路<br>(<span id="himeji_count"></span>件)
        </Tab>
        <Tab id="ooi" class="w-1/3 selction-btn text-xs">
          大井<br>(<span id="ooi_count"></span>件)
        </Tab>
        <Tab id="kanazawa" class="w-1/3 selction-btn text-xs">
          金沢<br>(<span id="kanazawa_count"></span>件)
        </Tab>
        <Tab id="monbetsu" class="w-1/3 selction-btn text-xs">
          門別<br>(<span id="monbetsu_count"></span>件)
        </Tab>
        <Tab id="morioka" class="w-1/3 selction-btn text-xs">
          盛岡<br>(<span id="morioka_count"></span>件)
        </Tab>
        <Tab id="obihiro" class="w-1/3 selction-btn text-xs">
          帯広<br>(<span id="obihiro_count"></span>件)
        </Tab>
      </TabList>

      <TabPanels id="tab_panel">
        <TabPanel id="p_kawasaki"><RegionRace area="川崎" :region="region" :day="day" :csv="data" /></TabPanel>
        <TabPanel id="p_nagoya"><RegionRace area="名古屋" :region="region" :day="day" :csv="data" /></TabPanel>
        <TabPanel id="p_kochi"><RegionRace area="高知" :region="region" :day="day" :csv="data" /></TabPanel>
        <TabPanel id="p_mizusawa"><RegionRace area="水沢" :region="region" :day="day" :csv="data" /></TabPanel>
        <TabPanel id="p_sonoda"><RegionRace area="園田" :region="region" :day="day" :csv="data" /></TabPanel>
